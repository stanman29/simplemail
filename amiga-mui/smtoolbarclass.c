...
STATIC ULONG SMToolbar_New(struct IClass *cl,Object *obj,struct opSet *msg)
{
	...
	if (!toolbar)
	{
		...
		if (horiz_group)
		{
			...
			for (i=0,j=0;i<button_count;i++)
			{
				if (!current_group)
				{
					if ((current_group = HGroup,
					     	...
					     	End))
					{
						DoMethod(horiz_group, OM_ADDMEMBER, (IPTR)current_group);
					} else
					{
				...
				if (buttons[i].pos != MUIV_SMToolbar_Space)
				{
					...
					if ((toolbar_buttons[i].obj = MakePictureButton(toolbar_buttons[i].text,name_buf)))
					{
						...
						DoMethod(current_group, OM_ADDMEMBER, (IPTR)toolbar_buttons[i].obj);
					} else
					{
						MUI_DisposeObject(toolbar);
						toolbar = NULL;
			...
			if (toolbar)
			{
				set(current_group, MUIA_Weight, 50 * j);
				if (add_hvspace)
					DoMethod(horiz_group, OM_ADDMEMBER, (IPTR)HVSpace);
			}
		}
	}
	...
	DoMethod(obj, OM_ADDMEMBER, (IPTR)toolbar);
...
STATIC ULONG SMToolbar_SetAttr(struct IClass *cl, Object *obj, struct MUIP_SMToolbar_SetAttr *msg)
{
	...
	switch (msg->attr)
	{
		case	MUIA_SMToolbar_Attr_Hide:
		    	...
		    	{
		    		DoMethod(data->toolbar, MUIM_TheBar_SetAttr, msg->id, MUIA_TheBar_Attr_Hide, msg->value);
		    	} else
		    	{
		    		for (i=0;i<data->button_count;i++)
		    		{
		...
		case	MUIA_SMToolbar_Attr_Disabled:
		    	if (data->used_thebar_mcc)
		    	{
		    		DoMethod(data->toolbar, MUIM_TheBar_SetAttr, msg->id, MUIA_TheBar_Attr_Disabled, msg->value);
		    	} else
		    	{
		    		for (i=0;i<data->button_count;i++)
		    		{
		...
		case	MUIA_SMToolbar_Attr_Selected:
		    	if (data->used_thebar_mcc)
		    	{
		    		DoMethod(data->toolbar, MUIM_TheBar_SetAttr, msg->id, MUIA_TheBar_Attr_Selected, msg->value);
		    	} else
		    	{
		    		for (i=0;i<data->button_count;i++)
		    		{
...
STATIC ULONG SMToolbar_GetAttr(struct IClass *cl, Object *obj, struct MUIP_SMToolbar_GetAttr *msg)
{
	...
	switch (msg->attr)
	{
		case	MUIA_SMToolbar_Attr_Hide:
		    	if (data->used_thebar_mcc)
		    	{
		    		DoMethod(data->toolbar, MUIM_TheBar_GetAttr, msg->id, MUIA_TheBar_Attr_Hide, (IPTR)msg->storage);
		    	} else
		    	{
		    		for (i=0;i<data->button_count;i++)
		    		{
		...
		case	MUIA_SMToolbar_Attr_Disabled:
		    	if (data->used_thebar_mcc)
		    	{
		    		DoMethod(data->toolbar, MUIM_TheBar_GetAttr, msg->id, MUIA_TheBar_Attr_Disabled, (IPTR)msg->storage);
		    	} else
		    	{
		    		for (i=0;i<data->button_count;i++)
		    		{
		...
		case	MUIA_SMToolbar_Attr_Selected:
		    	if (data->used_thebar_mcc)
		    	{
		    		DoMethod(data->toolbar, MUIM_TheBar_GetAttr, msg->id, MUIA_TheBar_Attr_Selected, (IPTR)msg->storage);
		    	} else
		    	{
		    		for (i=0;i<data->button_count;i++)
		    		{
...
STATIC ULONG SMToolbar_GetObject(struct IClass *cl, Object *obj, struct MUIP_SMToolbar_GetObject *msg)
{
	...
	{
		if (data->toolbar_buttons[i].ID == msg->id)
			return (IPTR)data->toolbar_buttons[i].obj;
	}
